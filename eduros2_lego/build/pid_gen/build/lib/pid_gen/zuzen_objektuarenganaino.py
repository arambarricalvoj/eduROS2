...
class ZuzenObjektuarenganaino(Node):
    def __init__(self):
        ...
        # Archivo CSV para registrar evolución
        ...
        with open(self.csv_file, mode="a", newline="") as f:
            writer = csv.writer(f)
            for i, kp in enumerate(self.population):
    ...
    def crear_individuo(self):
        # Inicialización aleatoria solo para la primera generación
        return random.uniform(4.0, 9.0)

    def crossover(self, parent1, parent2):
        # Cruce simple: promedio + ligera variación
        beta = random.random()  # factor de mezcla
        child = beta * parent1 + (1 - beta) * parent2
        return child

    def mutate(self, individual, mutation_rate=0.1):
        # Mutación: añadir ruido gaussiano
        if random.random() < mutation_rate:
            individual += random.uniform(-0.5, 0.5)
        # Mantener dentro del rango permitido
        return max(4.0, min(9.0, individual))

    def nueva_generacion(self, fitnesses):
        # Selección: ordenar por fitness (menor es mejor)
        sorted_pop = [x for _, x in sorted(zip(fitnesses, self.population))]
    ...
    def fitness_function(self, errors, outputs):
        ...
        # 2. Tiempo de recuperación (muestras fuera de ±5%)
    ...
    def ga_step(self):
        ...
        if not self.running_episode:
            if self.ind_index >= self.pop_size:
                # Nueva generación con fitness acumulados
                self.population = self.nueva_generacion(self.fitnesses)
                self.fitnesses = []  # limpiar para la siguiente
                self.ind_index = 0
                self.gen_index += 1
        ...
        if (time.time() - self.episode_start) >= self.episode_duration:
            ...
            self.fitnesses.append(fit)  # guardar fitness
