...
class ZuzenObjektuarenganaino(Node):
    def __init__(self):
        ...
        self.kp = 7.0#self.get_parameter('kp').value
        self.ki = 0.0#self.get_parameter('ki').value
        self.kd = 0.0#self.get_parameter('kd').value
    ...
    def mugitu_objektuarenganaino(self):
        ...
        self.derivada = error_percent - self.last_error

        correc_pid = (error_percent * self.kp) + (self.integral * self.ki) + (self.derivada * self.kd)
        self.get_logger().info(f"PID: {correc_pid}") 
        ...
        if self.ultrasoinu_distantzia > self.eten_distantzia:
            ...
            if not self.aurreko_abiadura:
                ...
                mezua.angular.z = self.abiadura * (correc_pid * 0.01)
