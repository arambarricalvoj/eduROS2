...
class Mando(Node):
    ...
    def normalizar_eje(self, valor_raw):
        deadzone = 2000
        if abs(valor_raw) < deadzone:
            return 0.0
        return valor_raw / 32767.0
        ...
        while rclpy.ok():
            try:
                ...
                for event in events:
                    ...
                    # Detectar eje X del joystick izquierdo
                    elif event.code == 'ABS_X':
                        msg.linear.x = -self.abiadura
                        #self.pub.publish(msg)
                    # Detectar eje Y del joystick izquierdo
                    elif event.code == 'ABS_Y':
                        self.get_logger().info(f"Y: {event.state}")
